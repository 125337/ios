...
/// 热度计（唤醒/崩溃排查，2026-09-24）：热路径 hook 只做原子计数（无字符串/无文件IO，零开销），
/// 每 2 秒汇总打点一条 [Heat] 速率日志（谁在烧唤醒一目了然）。排查闭环后摘除调用点。
/// 顺便记录 last_hook：崩溃瞬间由 signal/异常 handler 把崩溃类型+最后活跃 hook 写入 MioPlugin_Crash.log。
/// 用法：hook 入口调用 WPHeatTick("模块.方法");  key 必须是 ASCII 字符串字面量（指针需全程稳定，中文会乱码）。
