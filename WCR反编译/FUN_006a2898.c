// FUN_006a2898 @ 006a2898

void FUN_006a2898(undefined8 param_1)

{
  undefined *puVar1;
  undefined1 *puVar2;
  undefined1 *puVar3;
  undefined *puVar4;
  undefined *puVar5;
  undefined1 *local_58;
  undefined1 *local_50;
  undefined1 *local_48;
  undefined *local_40;
  undefined1 *local_38;
  undefined4 local_30;
  undefined *local_20;
  undefined1 *local_18;
  
  local_20 = (undefined1 *)0x0;
  _objc_storeStrong(&local_20,param_1);
  if ((local_20 == (undefined1 *)0x0) ||
     (puVar1 = local_20, (*(code *)PTR__objc_msgSend_02578628)(local_20,PTR_s_length_0269cca0),
     puVar1 == (undefined1 *)0x0)) {
    local_18 = (undefined1 *)0x0;
    local_30 = 1;
  }
  else {
    puVar2 = (undefined1 *)((long)&MACH_HEADER.cpusubtype + 1);
    _NSSearchPathForDirectoriesInDomains(9,1,1);
    _objc_retainAutoreleasedReturnValue();
    puVar3 = puVar2;
    (*(code *)PTR__objc_msgSend_02578628)();
    _objc_retainAutoreleasedReturnValue();
    local_38 = puVar3;
    (*(code *)PTR__objc_release_02578630)(puVar2);
    if ((local_38 == (undefined1 *)0x0) ||
       (puVar3 = local_38, (*(code *)PTR__objc_msgSend_02578628)(local_38,PTR_s_length_0269cca0),
       puVar3 == (undefined1 *)0x0)) {
      local_18 = (undefined1 *)0x0;
      local_30 = 1;
    }
    else {
      puVar1 = local_20;
      (*(code *)PTR__objc_msgSend_02578628)(local_20,PTR_s_hasPrefix__0269d320,&cf__);
      if (((ulong)puVar1 & 1) == 0) {
        puVar3 = local_38;
        (*(code *)PTR__objc_msgSend_02578628)
                  (local_38,PTR_s_stringByAppendingPathComponent__026cab30,local_20);
        _objc_retainAutoreleasedReturnValue();
        local_30 = 1;
        local_18 = puVar3;
      }
      else {
        puVar1 = local_20;
        puVar5 = PTR_s_rangeOfString__0269d838;
        (*(code *)PTR__objc_msgSend_02578628)
                  (local_20,PTR_s_rangeOfString__0269d838,&cf__Documents_);
        local_18 = local_20;
        local_48 = puVar1;
        local_40 = puVar5;
        if (puVar1 == (undefined1 *)0x7fffffffffffffff) {
          (*(code *)PTR__objc_retain_02578638)();
          local_30 = 1;
        }
        else {
          puVar4 = local_20;
          local_50 = puVar1 + (long)puVar5;
          (*(code *)PTR__objc_msgSend_02578628)(local_20,PTR_s_length_0269cca0);
          if (puVar1 + (long)puVar5 < puVar4) {
            puVar1 = local_20;
            (*(code *)PTR__objc_msgSend_02578628)
                      (local_20,PTR_s_substringFromIndex__0269d120,local_50);
            _objc_retainAutoreleasedReturnValue();
            puVar3 = local_38;
            local_58 = puVar1;
            (*(code *)PTR__objc_msgSend_02578628)
                      (local_38,PTR_s_stringByAppendingPathComponent__026cab30,puVar1);
            _objc_retainAutoreleasedReturnValue();
            local_30 = 1;
            local_18 = puVar3;
            _objc_storeStrong(&local_58,0);
          }
          else {
            local_18 = (undefined1 *)0x0;
            local_30 = 1;
          }
        }
      }
    }
    _objc_storeStrong(&local_38,0);
  }
  _objc_storeStrong(&local_20,0);
  _objc_autoreleaseReturnValue(local_18);
  return;
}

