// sanitizedUserNames: @ 009bb2c8

/* Function Stack Size: 0x18 bytes */

ID WCRefineAIStore::sanitizedUserNames_(ID param_1,SEL param_2,ID param_3)

{
  undefined *puVar1;
  ulong uVar2;
  undefined1 *puVar3;
  undefined *puVar4;
  undefined1 *puVar5;
  long lVar6;
  ulong uVar7;
  ulong local_190;
  ulong local_168;
  ulong local_160;
  ulong local_130;
  undefined1 auStack_128 [8];
  long local_120;
  long *local_118;
  ulong local_e8;
  undefined *local_e0;
  undefined1 *local_d8;
  int local_cc;
  ulong local_c8;
  SEL local_c0;
  ID local_b8;
  undefined1 *local_b0;
  undefined1 auStack_a8 [128];
  long local_28;
  
  local_28 = *(long *)PTR____stack_chk_guard_02578670;
  local_c8 = 0;
  local_c0 = param_2;
  local_b8 = param_1;
  _objc_storeStrong(&local_c8,param_3);
  uVar2 = local_c8;
  puVar1 = PTR__OBJC_CLASS___NSArray_026cdfe0;
  (*(code *)PTR__objc_msgSend_02578628)(PTR__OBJC_CLASS___NSArray_026cdfe0,PTR_s_class_0269cd60);
  (*(code *)PTR__objc_msgSend_02578628)(uVar2,PTR_s_isKindOfClass__0269cd68,puVar1);
  if ((uVar2 & 1) == 0) {
    puVar3 = *(undefined1 **)PTR____NSArray0___02578280;
    (*(code *)PTR__objc_retain_02578638)();
    local_cc = 1;
    local_b0 = puVar3;
  }
  else {
    puVar1 = PTR__OBJC_CLASS___NSMutableArray_026ce088;
    (*(code *)PTR__objc_msgSend_02578628)
              (PTR__OBJC_CLASS___NSMutableArray_026ce088,PTR_s_array_0269d158);
    _objc_retainAutoreleasedReturnValue();
    puVar4 = PTR__OBJC_CLASS___NSMutableSet_026ce0f0;
    local_d8 = puVar1;
    (*(code *)PTR__objc_msgSend_02578628)
              (PTR__OBJC_CLASS___NSMutableSet_026ce0f0,PTR_s_set_0269d510);
    _objc_retainAutoreleasedReturnValue();
    local_e0 = puVar4;
    _memset(auStack_128,0,0x40);
    uVar2 = local_c8;
    (*(code *)PTR__objc_retain_02578638)();
    local_160 = uVar2;
    (*(code *)PTR__objc_msgSend_02578628)
              (uVar2,PTR_s_countByEnumeratingWithState_obje_0269cd58,auStack_128,auStack_a8,0x10);
    if (local_160 != 0) {
      lVar6 = *local_118;
      local_168 = 0;
      do {
        do {
          if (*local_118 - lVar6 != 0) {
            _objc_enumerationMutation(*local_118 - lVar6,uVar2);
          }
          uVar7 = *(ulong *)(local_120 + local_168 * 8);
          puVar1 = PTR__OBJC_CLASS___NSString_026cdfe8;
          local_e8 = uVar7;
          (*(code *)PTR__objc_msgSend_02578628)
                    (PTR__OBJC_CLASS___NSString_026cdfe8,PTR_s_class_0269cd60);
          (*(code *)PTR__objc_msgSend_02578628)(uVar7,PTR_s_isKindOfClass__0269cd68,puVar1);
          if ((uVar7 & 1) == 0) {
            local_190 = 0;
          }
          else {
            local_190 = local_e8;
          }
          FUN_009af3fc();
          _objc_retainAutoreleasedReturnValue();
          local_130 = local_190;
          (*(code *)PTR__objc_msgSend_02578628)(local_190,PTR_s_length_0269cca0);
          if ((local_190 == 0) ||
             (puVar1 = local_e0,
             (*(code *)PTR__objc_msgSend_02578628)
                       (local_e0,PTR_s_containsObject__0269cbb8,local_130), ((ulong)puVar1 & 1) != 0
             )) {
            puVar3 = (undefined1 *)((long)&MACH_HEADER.magic + 3);
            local_cc = 3;
          }
          else {
            (*(code *)PTR__objc_msgSend_02578628)(local_e0,PTR_s_addObject__0269d180,local_130);
            (*(code *)PTR__objc_msgSend_02578628)(local_d8,PTR_s_addObject__0269d180,local_130);
            puVar5 = local_d8;
            (*(code *)PTR__objc_msgSend_02578628)(local_d8,PTR_s_count_0269cfe0);
            puVar3 = puVar5 + -2000;
            if (puVar5 < &section_00000798.reloff) {
              local_cc = 0;
            }
            else {
              puVar3 = (undefined1 *)((long)&MACH_HEADER.magic + 2);
              local_cc = 2;
            }
          }
          _objc_storeStrong(puVar3,&local_130,0);
          if ((local_cc != 0) && (local_cc != 3)) goto LAB_009bb654;
          local_168 = local_168 + 1;
        } while (local_168 < local_160);
        local_160 = uVar2;
        (*(code *)PTR__objc_msgSend_02578628)
                  (uVar2,PTR_s_countByEnumeratingWithState_obje_0269cd58,auStack_128,auStack_a8,0x10
                  );
        local_168 = 0;
      } while (local_160 != 0);
    }
    local_cc = 0;
LAB_009bb654:
    (*(code *)PTR__objc_release_02578630)(uVar2);
    puVar3 = local_d8;
    (*(code *)PTR__objc_msgSend_02578628)(local_d8,PTR_s_copy_0269d150);
    local_cc = 1;
    local_b0 = puVar3;
    _objc_storeStrong(&local_e0);
    _objc_storeStrong(&local_d8,0);
  }
  _objc_storeStrong(&local_c8,0);
  if (*(long *)PTR____stack_chk_guard_02578670 != local_28) {
                    /* WARNING: Subroutine does not return */
    ___stack_chk_fail();
  }
  _objc_autoreleaseReturnValue(0);
  return (ID)local_b0;
}

