// exportItems: @ 01b74708

/* Function Stack Size: 0x18 bytes */

void WCRefineLocalEmoticonLibraryViewController::exportItems_(ID param_1,SEL param_2,ID param_3)

{
  bool bVar1;
  undefined *puVar2;
  cfstringStruct *pcVar3;
  ulong uVar4;
  undefined *puVar5;
  long lVar6;
  dispatch_queue_t pdVar7;
  long lVar8;
  long lVar9;
  cfstringStruct *local_1d8;
  ulong local_198;
  ulong local_190;
  undefined *local_170;
  undefined4 local_168;
  undefined4 local_164;
  code *local_160;
  undefined *local_158;
  undefined *local_150;
  undefined *local_148;
  cfstringStruct *local_140;
  undefined *local_138;
  byte local_129;
  cfstringStruct *local_128;
  cfstringStruct *local_120;
  undefined1 auStack_118 [8];
  long local_110;
  long *local_108;
  long local_d8;
  undefined *local_d0;
  undefined4 local_c4;
  ulong local_c0;
  SEL local_b8;
  cfstringStruct *local_b0;
  undefined1 auStack_a8 [128];
  long local_28;
  
  local_28 = *(long *)PTR____stack_chk_guard_02578670;
  local_c0 = 0;
  local_b8 = param_2;
  local_b0 = (cfstringStruct *)param_1;
  _objc_storeStrong(&local_c0,param_3);
  uVar4 = local_c0;
  (*(code *)PTR__objc_msgSend_02578628)(local_c0,PTR_s_count_0269cfe0);
  if (uVar4 == 0) {
    (*(code *)PTR__objc_msgSend_02578628)
              (PTR_WCRefineHelper_026ce000,PTR_s_showModernToast__0269ce78,&cf_lgS_QvQ_);
    local_c4 = 1;
  }
  else {
    puVar5 = PTR__OBJC_CLASS___NSMutableArray_026ce088;
    (*(code *)PTR__objc_msgSend_02578628)
              (PTR__OBJC_CLASS___NSMutableArray_026ce088,PTR_s_array_0269d158);
    _objc_retainAutoreleasedReturnValue();
    local_d0 = puVar5;
    _memset(auStack_118,0,0x40);
    uVar4 = local_c0;
    (*(code *)PTR__objc_retain_02578638)();
    local_190 = uVar4;
    (*(code *)PTR__objc_msgSend_02578628)
              (uVar4,PTR_s_countByEnumeratingWithState_obje_0269cd58,auStack_118,auStack_a8,0x10);
    if (local_190 != 0) {
      lVar8 = *local_108;
      local_198 = 0;
      do {
        do {
          if (*local_108 - lVar8 != 0) {
            _objc_enumerationMutation(*local_108 - lVar8,uVar4);
          }
          lVar9 = *(long *)(local_110 + local_198 * 8);
          local_d8 = lVar9;
          (*(code *)PTR__objc_msgSend_02578628)(lVar9,PTR_s_relativePath_026ac370);
          _objc_retainAutoreleasedReturnValue();
          lVar6 = lVar9;
          (*(code *)PTR__objc_msgSend_02578628)();
          (*(code *)PTR__objc_release_02578630)(lVar9);
          puVar5 = local_d0;
          if (lVar6 != 0) {
            lVar6 = local_d8;
            (*(code *)PTR__objc_msgSend_02578628)(local_d8,PTR_s_relativePath_026ac370);
            _objc_retainAutoreleasedReturnValue();
            (*(code *)PTR__objc_msgSend_02578628)(puVar5,PTR_s_addObject__0269d180);
            (*(code *)PTR__objc_release_02578630)(lVar6);
          }
          local_198 = local_198 + 1;
        } while (local_198 < local_190);
        local_190 = uVar4;
        (*(code *)PTR__objc_msgSend_02578628)
                  (uVar4,PTR_s_countByEnumeratingWithState_obje_0269cd58,auStack_118,auStack_a8,0x10
                  );
        local_198 = 0;
      } while (local_190 != 0);
    }
    (*(code *)PTR__objc_release_02578630)(uVar4);
    puVar5 = local_d0;
    (*(code *)PTR__objc_msgSend_02578628)(local_d0,PTR_s_count_0269cfe0);
    if (puVar5 == (undefined *)0x0) {
      (*(code *)PTR__objc_msgSend_02578628)
                (PTR_WCRefineHelper_026ce000,PTR_s_showModernToast__0269ce78,&cf_lgS_QvQ_);
      local_c4 = 1;
    }
    else {
      local_129 = 0;
      bVar1 = (*(byte *)((long)&local_b0->field0_0x0 + (long)_exportSelectMode) & 1) == 0;
      if (bVar1) {
        local_1d8 = local_b0;
        (*(code *)PTR__objc_msgSend_02578628)(local_b0,PTR_s_currentExportGroupName_026bfb40);
        _objc_retainAutoreleasedReturnValue();
        local_128 = local_1d8;
      }
      else {
        local_1d8 = &cf__b;
      }
      local_129 = bVar1;
      (*(code *)PTR__objc_retain_02578638)();
      local_120 = local_1d8;
      if ((local_129 & 1) != 0) {
        (*(code *)PTR__objc_release_02578630)(local_128);
      }
      puVar5 = PTR_WCRefineLocalEmoticonStore_026ce440;
      (*(code *)PTR__objc_msgSend_02578628)
                (PTR_WCRefineLocalEmoticonStore_026ce440,
                 PTR_s_exportZipFileNameForPackName__026bfb48,local_120);
      _objc_retainAutoreleasedReturnValue();
      local_138 = puVar5;
      (*(code *)PTR__objc_msgSend_02578628)
                (PTR_WCRefineHelper_026ce000,PTR_s_showModernToast__0269ce78,&cf_ck_W_Q_);
      pdVar7 = _dispatch_get_global_queue(0,0);
      _objc_retainAutoreleasedReturnValue();
      puVar5 = local_138;
      local_170 = PTR___NSConcreteStackBlock_02578660;
      local_168 = 0xc2000000;
      local_164 = 0;
      local_160 = FUN_01b74c80;
      local_158 = &DAT_025839b0;
      (*(code *)PTR__objc_retain_02578638)();
      puVar2 = local_d0;
      local_150 = puVar5;
      (*(code *)PTR__objc_retain_02578638)();
      pcVar3 = local_b0;
      local_148 = puVar2;
      (*(code *)PTR__objc_retain_02578638)();
      local_140 = pcVar3;
      _dispatch_async(pdVar7,&local_170);
      (*(code *)PTR__objc_release_02578630)(pdVar7);
      _objc_storeStrong(&local_140);
      _objc_storeStrong(&local_148,0);
      _objc_storeStrong(&local_150,0);
      _objc_storeStrong(&local_138,0);
      _objc_storeStrong(&local_120,0);
      local_c4 = 0;
    }
    _objc_storeStrong(&local_d0,0);
  }
  _objc_storeStrong(&local_c0,0);
  if (*(long *)PTR____stack_chk_guard_02578670 - local_28 == 0) {
    return;
  }
                    /* WARNING: Subroutine does not return */
  ___stack_chk_fail(*(long *)PTR____stack_chk_guard_02578670 - local_28);
}

