// isDarkModeForView: @ 00eb375c

/* WARNING: Removing unreachable block (ram,0x00eb38b4) */
/* Function Stack Size: 0x18 bytes */

bool WCRefineAvatarFramePageOverlayHelper::isDarkModeForView_(ID param_1,SEL param_2,ID param_3)

{
  undefined *puVar1;
  undefined1 *puVar2;
  undefined1 *local_58;
  undefined1 *local_40;
  undefined1 *local_38;
  undefined *local_30;
  SEL local_28;
  ID local_20;
  byte local_11;
  
  local_30 = (undefined1 *)0x0;
  local_28 = param_2;
  local_20 = param_1;
  _objc_storeStrong(&local_30,param_3);
  puVar1 = local_30;
  (*(code *)PTR__objc_msgSend_02578628)(local_30,PTR_s_traitCollection_026caba8);
  _objc_retainAutoreleasedReturnValue();
  local_58 = puVar1;
  if (puVar1 == (undefined1 *)0x0) {
    local_40 = PTR__OBJC_CLASS___UITraitCollection_026ce138;
    (*(code *)PTR__objc_msgSend_02578628)
              (PTR__OBJC_CLASS___UITraitCollection_026ce138,PTR_s_currentTraitCollection_026ca5d8);
    _objc_retainAutoreleasedReturnValue();
    local_58 = local_40;
  }
  (*(code *)PTR__objc_retain_02578638)();
  local_38 = local_58;
  if (puVar1 == (undefined1 *)0x0) {
    (*(code *)PTR__objc_release_02578630)(local_40);
  }
  (*(code *)PTR__objc_release_02578630)(puVar1);
  puVar2 = local_38;
  (*(code *)PTR__objc_msgSend_02578628)(local_38,PTR_s_userInterfaceStyle_026cabc8);
  local_11 = puVar2 == (undefined1 *)((long)&MACH_HEADER.magic + 2);
  _objc_storeStrong(&local_38,0);
  _objc_storeStrong(&local_30,0);
  return local_11 & 1;
}

