// FUN_0010a464 @ 0010a464

void FUN_0010a464(undefined8 param_1)

{
  bool bVar1;
  cfstringStruct *pcVar2;
  ulong uVar3;
  ulong uVar4;
  undefined *puVar5;
  ulong local_40 [3];
  cfstringStruct *local_28;
  ulong local_20;
  ulong local_18;
  
  local_20 = 0;
  _objc_storeStrong(&local_20,param_1);
  pcVar2 = &cf_Alpha0_6;
  _NSSelectorFromString();
  local_28 = pcVar2;
  if ((local_20 != 0) &&
     (uVar3 = local_20,
     (*(code *)PTR__objc_msgSend_02578628)(local_20,PTR_s_respondsToSelector__026ca818,pcVar2),
     (uVar3 & 1) != 0)) {
    uVar4 = local_20;
    (*(code *)PTR__objc_msgSend_02578628)(local_20,local_28);
    _objc_retainAutoreleasedReturnValue();
    puVar5 = PTR__OBJC_CLASS___UIColor_026cdf78;
    local_40[0] = uVar4;
    (*(code *)PTR__objc_msgSend_02578628)(PTR__OBJC_CLASS___UIColor_026cdf78,PTR_s_class_0269cd60);
    (*(code *)PTR__objc_msgSend_02578628)(uVar4,PTR_s_isKindOfClass__0269cd68,puVar5);
    uVar3 = local_40[0];
    bVar1 = (uVar4 & 1) != 0;
    if (bVar1) {
      (*(code *)PTR__objc_retain_02578638)();
      local_18 = uVar3;
    }
    _objc_storeStrong(local_40,0);
    if (bVar1) goto LAB_0010a620;
  }
  uVar3 = local_20;
  (*(code *)PTR__objc_msgSend_02578628)
            (0x3fe3333333333333,local_20,PTR_s_colorWithAlphaComponent__026ca578);
  _objc_retainAutoreleasedReturnValue();
  local_18 = uVar3;
LAB_0010a620:
  _objc_storeStrong(&local_20,0);
  _objc_autoreleaseReturnValue(local_18);
  return;
}

