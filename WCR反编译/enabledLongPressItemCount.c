// enabledLongPressItemCount @ 011001a4

/* Function Stack Size: 0x10 bytes */

unsigned_long_long WCRefinePluginIconCatalog::enabledLongPressItemCount(ID param_1,SEL param_2)

{
  unsigned_long_long uVar1;
  ID IVar2;
  ID IVar3;
  long lVar4;
  long lVar5;
  ulong local_138;
  ulong local_130;
  undefined1 auStack_110 [8];
  long local_108;
  long *local_100;
  long local_d0;
  unsigned_long_long local_c8;
  ID local_c0;
  SEL local_b8;
  ID local_b0;
  undefined1 auStack_a8 [128];
  long local_28;
  
  local_28 = *(long *)PTR____stack_chk_guard_02578670;
  local_b8 = param_2;
  local_b0 = param_1;
  (*(code *)PTR__objc_msgSend_02578628)(param_1,PTR_s_disabledLongPressItemIDSet_026aedd8);
  _objc_retainAutoreleasedReturnValue();
  local_c8 = 0;
  local_c0 = param_1;
  _memset(auStack_110,0,0x40);
  IVar2 = local_b0;
  (*(code *)PTR__objc_msgSend_02578628)(local_b0,PTR_s_resolvedLongPressOrder_026aed60);
  _objc_retainAutoreleasedReturnValue();
  local_130 = IVar2;
  (*(code *)PTR__objc_msgSend_02578628)();
  if (local_130 != 0) {
    lVar4 = *local_100;
    local_138 = 0;
    do {
      do {
        if (*local_100 - lVar4 != 0) {
          _objc_enumerationMutation(*local_100 - lVar4,IVar2);
        }
        lVar5 = *(long *)(local_108 + local_138 * 8);
        local_d0 = lVar5;
        (*(code *)PTR__objc_msgSend_02578628)(lVar5,PTR_s_length_0269cca0);
        if ((lVar5 != 0) &&
           (IVar3 = local_c0,
           (*(code *)PTR__objc_msgSend_02578628)(local_c0,PTR_s_containsObject__0269cbb8,local_d0),
           (IVar3 & 1) == 0)) {
          local_c8 = local_c8 + 1;
        }
        local_138 = local_138 + 1;
      } while (local_138 < local_130);
      local_130 = IVar2;
      (*(code *)PTR__objc_msgSend_02578628)
                (IVar2,PTR_s_countByEnumeratingWithState_obje_0269cd58,auStack_110,auStack_a8,0x10);
      local_138 = 0;
    } while (local_130 != 0);
  }
  (*(code *)PTR__objc_release_02578630)(IVar2);
  uVar1 = local_c8;
  _objc_storeStrong(&local_c0,0);
  if (*(long *)PTR____stack_chk_guard_02578670 - local_28 == 0) {
    return uVar1;
  }
                    /* WARNING: Subroutine does not return */
  ___stack_chk_fail(*(long *)PTR____stack_chk_guard_02578670 - local_28);
}

