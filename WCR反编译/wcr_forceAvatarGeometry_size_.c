// wcr_forceAvatarGeometry:size: @ 01adccb8

/* Function Stack Size: 0x20 bytes */

void WCRefineHomeAvatarStripEditorViewController::wcr_forceAvatarGeometry_size_
               (ID param_1,SEL param_2,ID param_3,double param_4)

{
  ulong uVar1;
  undefined *puVar2;
  ulong uVar3;
  cfstringStruct *pcVar4;
  undefined8 uVar5;
  double dVar6;
  undefined8 uVar7;
  double dVar8;
  undefined1 auStack_a8 [48];
  undefined1 auStack_78 [52];
  undefined4 local_44;
  double local_40;
  ulong local_38;
  SEL local_30;
  ID local_28;
  
  local_38 = 0;
  local_30 = param_2;
  local_28 = param_1;
  _objc_storeStrong(&local_38,param_3);
  uVar3 = local_38;
  puVar2 = PTR__OBJC_CLASS___UIView_026cdfd8;
  local_40 = param_4;
  (*(code *)PTR__objc_msgSend_02578628)(PTR__OBJC_CLASS___UIView_026cdfd8,PTR_s_class_0269cd60);
  (*(code *)PTR__objc_msgSend_02578628)(uVar3,PTR_s_isKindOfClass__0269cd68,puVar2);
  if (((uVar3 & 1) == 0) || (local_40 < 1.0)) {
    local_44 = 1;
  }
  else {
    _memcpy(auStack_78,PTR__CGAffineTransformIdentity_025782d8,0x30);
    uVar3 = local_38;
    _memcpy(auStack_a8,auStack_78,0x30);
    (*(code *)PTR__objc_msgSend_02578628)(uVar3,PTR_s_setTransform__026caad0,auStack_a8);
    uVar7 = 0;
    uVar5 = 0;
    dVar6 = local_40;
    dVar8 = local_40;
    FUN_01add21c();
    (*(code *)PTR__objc_msgSend_02578628)
              (uVar5,uVar7,dVar6,dVar8,local_38,PTR_s_setBounds__026ca8a0);
    uVar7 = 0;
    uVar5 = uVar7;
    dVar6 = local_40;
    dVar8 = local_40;
    FUN_01add21c();
    (*(code *)PTR__objc_msgSend_02578628)(uVar7,uVar5,dVar6,dVar8,local_38,PTR_s_setFrame__026ca960)
    ;
    uVar3 = local_38;
    pcVar4 = &cf_setImageSize_;
    _NSSelectorFromString(&cf_setImageSize_);
    (*(code *)PTR__objc_msgSend_02578628)(uVar3,PTR_s_respondsToSelector__026ca818,pcVar4);
    uVar1 = local_38;
    if ((uVar3 & 1) != 0) {
      pcVar4 = &cf_setImageSize_;
      _NSSelectorFromString();
      dVar6 = local_40;
      dVar8 = local_40;
      FUN_01add268();
      (*(code *)PTR__objc_msgSend_02578628)(dVar6,dVar8,uVar1,pcVar4);
    }
    uVar3 = local_38;
    pcVar4 = &cf_setConerSize_;
    _NSSelectorFromString(&cf_setConerSize_);
    (*(code *)PTR__objc_msgSend_02578628)(uVar3,PTR_s_respondsToSelector__026ca818,pcVar4);
    uVar1 = local_38;
    if ((uVar3 & 1) != 0) {
      pcVar4 = &cf_setConerSize_;
      _NSSelectorFromString();
      (*(code *)PTR__objc_msgSend_02578628)(uVar1,pcVar4,0);
    }
    uVar3 = local_38;
    pcVar4 = &cf_setBRoundCorner_;
    _NSSelectorFromString(&cf_setBRoundCorner_);
    (*(code *)PTR__objc_msgSend_02578628)(uVar3,PTR_s_respondsToSelector__026ca818,pcVar4);
    uVar1 = local_38;
    if ((uVar3 & 1) != 0) {
      pcVar4 = &cf_setBRoundCorner_;
      _NSSelectorFromString();
      (*(code *)PTR__objc_msgSend_02578628)(uVar1,pcVar4,0);
    }
    (*(code *)PTR__objc_msgSend_02578628)(local_38,PTR_s_setClipsToBounds__026ca8c8,1);
    uVar3 = local_38;
    (*(code *)PTR__objc_msgSend_02578628)(local_38,PTR_s_layer_026ca788);
    _objc_retainAutoreleasedReturnValue();
    (*(code *)PTR__objc_msgSend_02578628)(0);
    (*(code *)PTR__objc_release_02578630)(uVar3);
    uVar3 = local_38;
    (*(code *)PTR__objc_msgSend_02578628)(local_38,PTR_s_layer_026ca788);
    _objc_retainAutoreleasedReturnValue();
    (*(code *)PTR__objc_msgSend_02578628)();
    (*(code *)PTR__objc_release_02578630)(uVar3);
    (*(code *)PTR__objc_msgSend_02578628)(local_38,PTR_s_setNeedsLayout_0269deb8);
    (*(code *)PTR__objc_msgSend_02578628)(local_38,PTR_s_layoutIfNeeded_026ca790);
    local_44 = 0;
  }
  _objc_storeStrong(&local_38,0);
  return;
}

