// deduplicatedFontItems: @ 019d0ea4

/* Function Stack Size: 0x18 bytes */

ID WCRefineFontListViewController::deduplicatedFontItems_(ID param_1,SEL param_2,ID param_3)

{
  undefined *puVar1;
  undefined *puVar2;
  long lVar3;
  cfstringStruct *pcVar4;
  cfstringStruct *local_180;
  ulong local_158;
  ulong local_150;
  ulong local_138;
  cfstringStruct *local_120;
  undefined1 auStack_118 [8];
  long local_110;
  long *local_108;
  cfstringStruct *local_d8;
  undefined *local_d0;
  undefined *local_c8;
  ulong local_c0;
  SEL local_b8;
  ID local_b0;
  undefined1 auStack_a8 [128];
  long local_28;
  
  local_28 = *(long *)PTR____stack_chk_guard_02578670;
  local_c0 = 0;
  local_b8 = param_2;
  local_b0 = param_1;
  _objc_storeStrong(&local_c0,param_3);
  puVar1 = PTR__OBJC_CLASS___NSMutableArray_026ce088;
  (*(code *)PTR__objc_msgSend_02578628)
            (PTR__OBJC_CLASS___NSMutableArray_026ce088,PTR_s_array_0269d158);
  _objc_retainAutoreleasedReturnValue();
  puVar2 = PTR__OBJC_CLASS___NSMutableSet_026ce0f0;
  local_c8 = puVar1;
  (*(code *)PTR__objc_msgSend_02578628)(PTR__OBJC_CLASS___NSMutableSet_026ce0f0,PTR_s_set_0269d510);
  _objc_retainAutoreleasedReturnValue();
  local_d0 = puVar2;
  _memset(auStack_118,0,0x40);
  if (local_c0 == 0) {
    local_138 = *(ulong *)PTR____NSArray0___02578280;
  }
  else {
    local_138 = local_c0;
  }
  (*(code *)PTR__objc_retain_02578638)();
  local_150 = local_138;
  (*(code *)PTR__objc_msgSend_02578628)
            (local_138,PTR_s_countByEnumeratingWithState_obje_0269cd58,auStack_118,auStack_a8,0x10);
  if (local_150 != 0) {
    lVar3 = *local_108;
    local_158 = 0;
    do {
      do {
        if (*local_108 - lVar3 != 0) {
          _objc_enumerationMutation(*local_108 - lVar3,local_138);
        }
        pcVar4 = *(cfstringStruct **)(local_110 + local_158 * 8);
        local_d8 = pcVar4;
        FUN_019d1290();
        _objc_retainAutoreleasedReturnValue();
        local_120 = pcVar4;
        (*(code *)PTR__objc_msgSend_02578628)(pcVar4,PTR_s_length_0269cca0);
        if (pcVar4 == (cfstringStruct *)0x0) {
          pcVar4 = local_d8;
          (*(code *)PTR__objc_msgSend_02578628)
                    (local_d8,PTR_s_objectForKeyedSubscript__0269d098,&cf_postScriptName);
          _objc_retainAutoreleasedReturnValue();
          local_180 = pcVar4;
          if (pcVar4 == (cfstringStruct *)0x0) {
            local_180 = &::cf___;
          }
          _objc_storeStrong(&local_120,local_180);
          (*(code *)PTR__objc_release_02578630)(pcVar4);
        }
        pcVar4 = local_120;
        (*(code *)PTR__objc_msgSend_02578628)(local_120,PTR_s_length_0269cca0);
        if ((pcVar4 == (cfstringStruct *)0x0) ||
           (puVar1 = local_d0,
           (*(code *)PTR__objc_msgSend_02578628)(local_d0,PTR_s_containsObject__0269cbb8,local_120),
           ((ulong)puVar1 & 1) == 0)) {
          pcVar4 = local_120;
          (*(code *)PTR__objc_msgSend_02578628)(local_120,PTR_s_length_0269cca0);
          if (pcVar4 != (cfstringStruct *)0x0) {
            (*(code *)PTR__objc_msgSend_02578628)(local_d0,PTR_s_addObject__0269d180,local_120);
          }
          (*(code *)PTR__objc_msgSend_02578628)(local_c8,PTR_s_addObject__0269d180,local_d8);
        }
        _objc_storeStrong(&local_120,0);
        local_158 = local_158 + 1;
      } while (local_158 < local_150);
      local_150 = local_138;
      (*(code *)PTR__objc_msgSend_02578628)
                (local_138,PTR_s_countByEnumeratingWithState_obje_0269cd58,auStack_118,auStack_a8,
                 0x10);
      local_158 = 0;
    } while (local_150 != 0);
  }
  (*(code *)PTR__objc_release_02578630)(local_138);
  puVar1 = local_c8;
  (*(code *)PTR__objc_msgSend_02578628)(local_c8,PTR_s_copy_0269d150);
  _objc_storeStrong(&local_d0);
  _objc_storeStrong(&local_c8,0);
  _objc_storeStrong(&local_c0,0);
  if (*(long *)PTR____stack_chk_guard_02578670 == local_28) {
    _objc_autoreleaseReturnValue(0);
    return (ID)puVar1;
  }
                    /* WARNING: Subroutine does not return */
  ___stack_chk_fail();
}

