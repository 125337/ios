// FUN_007337ec @ 007337ec

byte FUN_007337ec(undefined8 param_1,undefined8 param_2)

{
  bool bVar1;
  undefined *puVar2;
  ulong uVar3;
  ulong uVar4;
  long lVar5;
  long lVar6;
  ulong uVar7;
  ulong local_2f0;
  ulong local_2e8;
  ulong local_290;
  ulong local_288;
  long local_220;
  long local_218;
  undefined *local_210;
  long local_208;
  undefined *local_200;
  long local_1f8;
  undefined1 auStack_1f0 [8];
  long local_1e8;
  long *local_1e0;
  long local_1b0;
  ulong local_1a8;
  undefined1 auStack_1a0 [8];
  long local_198;
  long *local_190;
  undefined8 local_160;
  ulong local_158;
  int local_150;
  ulong local_140;
  ulong local_138;
  byte local_129;
  undefined1 auStack_128 [128];
  undefined1 auStack_a8 [128];
  long local_28;
  
  local_28 = *(long *)PTR____stack_chk_guard_02578670;
  local_138 = 0;
  _objc_storeStrong(&local_138,param_1);
  local_140 = 0;
  _objc_storeStrong(&local_140,param_2);
  uVar3 = local_138;
  puVar2 = PTR__OBJC_CLASS___NSString_026cdfe8;
  (*(code *)PTR__objc_msgSend_02578628)(PTR__OBJC_CLASS___NSString_026cdfe8,PTR_s_class_0269cd60);
  (*(code *)PTR__objc_msgSend_02578628)(uVar3,PTR_s_isKindOfClass__0269cd68,puVar2);
  if ((((uVar3 & 1) == 0) ||
      (uVar3 = local_138, (*(code *)PTR__objc_msgSend_02578628)(local_138,PTR_s_length_0269cca0),
      uVar3 == 0)) ||
     (uVar3 = local_140, (*(code *)PTR__objc_msgSend_02578628)(local_140,PTR_s_count_0269cfe0),
     uVar3 == 0)) {
    local_129 = 0;
    local_150 = 1;
    goto LAB_00733fcc;
  }
  uVar3 = local_138;
  FUN_00734994();
  _objc_retainAutoreleasedReturnValue();
  local_158 = uVar3;
  (*(code *)PTR__objc_msgSend_02578628)(uVar3,PTR_s_length_0269cca0);
  if (uVar3 == 0) {
LAB_00733b44:
    uVar3 = local_138;
    FUN_00734e44(0);
    _objc_retainAutoreleasedReturnValue();
    local_1a8 = uVar3;
    (*(code *)PTR__objc_msgSend_02578628)(uVar3,PTR_s_length_0269cca0);
    if (uVar3 == 0) {
      local_129 = 0;
      local_150 = 1;
    }
    else {
      _memset(auStack_1f0,0,0x40);
      uVar3 = local_140;
      (*(code *)PTR__objc_retain_02578638)();
      local_2e8 = uVar3;
      (*(code *)PTR__objc_msgSend_02578628)
                (uVar3,PTR_s_countByEnumeratingWithState_obje_0269cd58,auStack_1f0,auStack_128,0x10)
      ;
      if (local_2e8 != 0) {
        lVar5 = *local_1e0;
        local_2f0 = 0;
        do {
          do {
            if (*local_1e0 - lVar5 != 0) {
              _objc_enumerationMutation(*local_1e0 - lVar5,uVar3);
            }
            lVar6 = *(long *)(local_1e8 + local_2f0 * 8);
            local_1b0 = lVar6;
            FUN_007350b8();
            _objc_retainAutoreleasedReturnValue();
            local_1f8 = lVar6;
            (*(code *)PTR__objc_msgSend_02578628)(lVar6,PTR_s_length_0269cca0);
            if (lVar6 == 0) {
              local_150 = 5;
            }
            else {
              lVar6 = local_1f8;
              puVar2 = PTR_s_rangeOfString__0269d838;
              (*(code *)PTR__objc_msgSend_02578628)(local_1f8,PTR_s_rangeOfString__0269d838,&cf____)
              ;
              bVar1 = true;
              local_208 = lVar6;
              local_200 = puVar2;
              if (lVar6 == 0x7fffffffffffffff) {
                lVar6 = local_1f8;
                puVar2 = PTR_s_rangeOfString__0269d838;
                (*(code *)PTR__objc_msgSend_02578628)
                          (local_1f8,PTR_s_rangeOfString__0269d838,&cf___);
                bVar1 = lVar6 != 0x7fffffffffffffff;
                local_218 = lVar6;
                local_210 = puVar2;
              }
              if (bVar1) {
                local_150 = 5;
              }
              else {
                lVar6 = local_1f8;
                FUN_00734e44();
                _objc_retainAutoreleasedReturnValue();
                local_220 = lVar6;
                (*(code *)PTR__objc_msgSend_02578628)(lVar6,PTR_s_length_0269cca0);
                uVar7 = 0;
                if (lVar6 == 0) {
LAB_00733e74:
                  local_150 = 0;
                }
                else {
                  uVar4 = local_1a8;
                  (*(code *)PTR__objc_msgSend_02578628)
                            (local_1a8,PTR_s_isEqualToString__0269ccc8,local_220);
                  uVar7 = uVar4 & 0xffffffff;
                  if ((uVar4 & 1) == 0) goto LAB_00733e74;
                  uVar7 = 1;
                  local_129 = 1;
                  local_150 = 1;
                }
                _objc_storeStrong(uVar7,&local_220,0);
              }
            }
            _objc_storeStrong(&local_1f8,0);
            if ((local_150 != 0) && (local_150 != 5)) goto LAB_00733f2c;
            local_2f0 = local_2f0 + 1;
          } while (local_2f0 < local_2e8);
          local_2e8 = uVar3;
          (*(code *)PTR__objc_msgSend_02578628)
                    (uVar3,PTR_s_countByEnumeratingWithState_obje_0269cd58,auStack_1f0,auStack_128,
                     0x10);
          local_2f0 = 0;
        } while (local_2e8 != 0);
      }
      local_150 = 0;
LAB_00733f2c:
      (*(code *)PTR__objc_release_02578630)(uVar3);
      if (local_150 == 0) {
        local_129 = 0;
        local_150 = 1;
      }
    }
    _objc_storeStrong(&local_1a8,0);
  }
  else {
    _memset(auStack_1a0,0,0x40);
    uVar3 = local_140;
    (*(code *)PTR__objc_retain_02578638)();
    local_288 = uVar3;
    (*(code *)PTR__objc_msgSend_02578628)
              (uVar3,PTR_s_countByEnumeratingWithState_obje_0269cd58,auStack_1a0,auStack_a8,0x10);
    if (local_288 != 0) {
      lVar5 = *local_190;
      local_290 = 0;
      do {
        do {
          if (*local_190 - lVar5 != 0) {
            _objc_enumerationMutation(*local_190 - lVar5,uVar3);
          }
          local_160 = *(undefined8 *)(local_198 + local_290 * 8);
          uVar7 = local_158;
          (*(code *)PTR__objc_msgSend_02578628)(local_158,PTR_s_isEqualToString__0269ccc8,local_160)
          ;
          if ((uVar7 & 1) != 0) {
            local_129 = 1;
            local_150 = 1;
            goto LAB_00733b20;
          }
          local_290 = local_290 + 1;
        } while (local_290 < local_288);
        local_288 = uVar3;
        (*(code *)PTR__objc_msgSend_02578628)
                  (uVar3,PTR_s_countByEnumeratingWithState_obje_0269cd58,auStack_1a0,auStack_a8,0x10
                  );
        local_290 = 0;
      } while (local_288 != 0);
    }
    local_150 = 0;
LAB_00733b20:
    (*(code *)PTR__objc_release_02578630)(uVar3);
    if (local_150 == 0) goto LAB_00733b44;
  }
  _objc_storeStrong(&local_158,0);
LAB_00733fcc:
  _objc_storeStrong(&local_140);
  _objc_storeStrong(&local_138,0);
  if (*(long *)PTR____stack_chk_guard_02578670 - local_28 == 0) {
    return local_129 & 1;
  }
                    /* WARNING: Subroutine does not return */
  ___stack_chk_fail(*(long *)PTR____stack_chk_guard_02578670 - local_28);
}

