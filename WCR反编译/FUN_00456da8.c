// FUN_00456da8 @ 00456da8

void FUN_00456da8(undefined8 param_1)

{
  undefined *puVar1;
  ulong uVar2;
  long lVar3;
  ulong local_190;
  ulong local_188;
  ulong local_140;
  undefined1 auStack_120 [8];
  long local_118;
  long *local_110;
  undefined8 local_e0;
  undefined4 local_d4;
  ulong local_d0 [3];
  ulong local_b8;
  ulong local_b0;
  undefined1 auStack_a8 [128];
  long local_28;
  
  local_28 = *(long *)PTR____stack_chk_guard_02578670;
  local_b0 = 0;
  _objc_storeStrong(&local_b0,param_1);
  uVar2 = local_b0;
  puVar1 = PTR__OBJC_CLASS___UIView_026cdfd8;
  (*(code *)PTR__objc_msgSend_02578628)(PTR__OBJC_CLASS___UIView_026cdfd8,PTR_s_class_0269cd60);
  (*(code *)PTR__objc_msgSend_02578628)(uVar2,PTR_s_isKindOfClass__0269cd68,puVar1);
  if ((uVar2 & 1) == 0) {
    local_140 = 0;
  }
  else {
    local_140 = local_b0;
  }
  (*(code *)PTR__objc_retain_02578638)();
  local_b8 = local_140;
  (*(code *)PTR__objc_msgSend_02578628)(local_140,PTR_s_viewWithTag__026cabe0,0x2c63f);
  _objc_retainAutoreleasedReturnValue();
  puVar1 = PTR__OBJC_CLASS___UIScrollView_026ce1a0;
  local_d0[0] = local_140;
  (*(code *)PTR__objc_msgSend_02578628)
            (PTR__OBJC_CLASS___UIScrollView_026ce1a0,PTR_s_class_0269cd60);
  (*(code *)PTR__objc_msgSend_02578628)(local_140,PTR_s_isKindOfClass__0269cd68,puVar1);
  if ((local_140 & 1) == 0) {
    local_d4 = 1;
  }
  else {
    _memset(auStack_120,0,0x40);
    uVar2 = local_d0[0];
    (*(code *)PTR__objc_msgSend_02578628)(local_d0[0],PTR_s_subviews_026cab40);
    _objc_retainAutoreleasedReturnValue();
    local_188 = uVar2;
    (*(code *)PTR__objc_msgSend_02578628)();
    if (local_188 != 0) {
      lVar3 = *local_110;
      local_190 = 0;
      do {
        do {
          if (*local_110 - lVar3 != 0) {
            _objc_enumerationMutation(*local_110 - lVar3,uVar2);
          }
          local_e0 = *(undefined8 *)(local_118 + local_190 * 8);
          FUN_00457ac4(local_e0);
          local_190 = local_190 + 1;
        } while (local_190 < local_188);
        local_188 = uVar2;
        (*(code *)PTR__objc_msgSend_02578628)
                  (uVar2,PTR_s_countByEnumeratingWithState_obje_0269cd58,auStack_120,auStack_a8,0x10
                  );
        local_190 = 0;
      } while (local_188 != 0);
    }
    (*(code *)PTR__objc_release_02578630)(uVar2);
    local_d4 = 0;
  }
  _objc_storeStrong(local_d0);
  _objc_storeStrong(&local_b8,0);
  _objc_storeStrong(&local_b0,0);
  if (*(long *)PTR____stack_chk_guard_02578670 - local_28 == 0) {
    return;
  }
                    /* WARNING: Subroutine does not return */
  ___stack_chk_fail(*(long *)PTR____stack_chk_guard_02578670 - local_28);
}

