// setPendingSaveSourcePaths:preferredName: @ 01979734

/* WARNING: Restarted to delay deadcode elimination for space: stack */
/* Function Stack Size: 0x20 bytes */

void WCRFileManagerState::setPendingSaveSourcePaths_preferredName_
               (ID param_1,SEL param_2,ID param_3,ID param_4)

{
  undefined *puVar1;
  long lVar2;
  ulong uVar3;
  long local_1b8;
  undefined *local_1b0;
  ulong local_178;
  ulong local_170;
  ulong local_158;
  long local_130;
  undefined *local_120;
  undefined1 auStack_118 [8];
  long local_110;
  long *local_108;
  ulong local_d8;
  undefined *local_d0;
  long local_c8;
  ulong local_c0;
  SEL local_b8;
  ID local_b0;
  undefined1 auStack_a8 [128];
  long local_28;
  
  local_28 = *(long *)PTR____stack_chk_guard_02578670;
  local_c0 = 0;
  local_b8 = param_2;
  local_b0 = param_1;
  _objc_storeStrong(&local_c0,param_3);
  local_c8 = 0;
  _objc_storeStrong(&local_c8,param_4);
  puVar1 = PTR__OBJC_CLASS___NSMutableArray_026ce088;
  (*(code *)PTR__objc_msgSend_02578628)
            (PTR__OBJC_CLASS___NSMutableArray_026ce088,PTR_s_array_0269d158);
  _objc_retainAutoreleasedReturnValue();
  local_d0 = puVar1;
  _memset(auStack_118,0,0x40);
  if (local_c0 == 0) {
    local_158 = *(ulong *)PTR____NSArray0___02578280;
  }
  else {
    local_158 = local_c0;
  }
  (*(code *)PTR__objc_retain_02578638)();
  local_170 = local_158;
  (*(code *)PTR__objc_msgSend_02578628)
            (local_158,PTR_s_countByEnumeratingWithState_obje_0269cd58,auStack_118,auStack_a8,0x10);
  if (local_170 != 0) {
    lVar2 = *local_108;
    local_178 = 0;
    do {
      do {
        if (*local_108 - lVar2 != 0) {
          _objc_enumerationMutation(*local_108 - lVar2,local_158);
        }
        uVar3 = *(ulong *)(local_110 + local_178 * 8);
        puVar1 = PTR__OBJC_CLASS___NSString_026cdfe8;
        local_d8 = uVar3;
        (*(code *)PTR__objc_msgSend_02578628)
                  (PTR__OBJC_CLASS___NSString_026cdfe8,PTR_s_class_0269cd60);
        (*(code *)PTR__objc_msgSend_02578628)(uVar3,PTR_s_isKindOfClass__0269cd68,puVar1);
        if (((uVar3 & 1) != 0) &&
           (uVar3 = local_d8, (*(code *)PTR__objc_msgSend_02578628)(local_d8,PTR_s_length_0269cca0),
           puVar1 = local_d0, uVar3 != 0)) {
          uVar3 = local_d8;
          (*(code *)PTR__objc_msgSend_02578628)(local_d8,PTR_s_stringByStandardizingPath_026cab38);
          _objc_retainAutoreleasedReturnValue();
          (*(code *)PTR__objc_msgSend_02578628)(puVar1,PTR_s_addObject__0269d180);
          (*(code *)PTR__objc_release_02578630)(uVar3);
        }
        local_178 = local_178 + 1;
      } while (local_178 < local_170);
      local_170 = local_158;
      (*(code *)PTR__objc_msgSend_02578628)
                (local_158,PTR_s_countByEnumeratingWithState_obje_0269cd58,auStack_118,auStack_a8,
                 0x10);
      local_178 = 0;
    } while (local_170 != 0);
  }
  (*(code *)PTR__objc_release_02578630)(local_158);
  puVar1 = local_d0;
  (*(code *)PTR__objc_msgSend_02578628)(local_d0,PTR_s_count_0269cfe0);
  if (puVar1 == (undefined *)0x0) {
    local_1b0 = (undefined *)0x0;
  }
  else {
    local_1b0 = local_d0;
    (*(code *)PTR__objc_msgSend_02578628)(local_d0,PTR_s_copy_0269d150);
    local_120 = local_1b0;
  }
  (*(code *)PTR__objc_msgSend_02578628)
            (local_b0,PTR_s_setPendingSaveSourcePaths__026ba210,local_1b0);
  if (puVar1 != (undefined *)0x0) {
    (*(code *)PTR__objc_release_02578630)(local_120);
  }
  lVar2 = local_c8;
  (*(code *)PTR__objc_msgSend_02578628)(local_c8,PTR_s_length_0269cca0);
  if (lVar2 == 0) {
    local_1b8 = 0;
  }
  else {
    local_1b8 = local_c8;
    (*(code *)PTR__objc_msgSend_02578628)(local_c8,PTR_s_lastPathComponent_026ca780);
    _objc_retainAutoreleasedReturnValue();
    local_130 = local_1b8;
  }
  (*(code *)PTR__objc_msgSend_02578628)
            (local_b0,PTR_s_setPendingSavePreferredName__026ba218,local_1b8);
  if (lVar2 != 0) {
    (*(code *)PTR__objc_release_02578630)(local_130);
  }
  _objc_storeStrong(&local_d0);
  _objc_storeStrong(&local_c8,0);
  _objc_storeStrong(&local_c0,0);
  if (*(long *)PTR____stack_chk_guard_02578670 - local_28 == 0) {
    return;
  }
                    /* WARNING: Subroutine does not return */
  ___stack_chk_fail(*(long *)PTR____stack_chk_guard_02578670 - local_28);
}

